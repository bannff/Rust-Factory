...
`evaluation-core` does not consume `workflow_core::Run` directly. It owns a versioned `TerminalEvidenceSnapshotV1` projection and reads it through a tenant-scoped port. Canonical bytes are built only in core using length-prefixed UTF-8 fields; tenant scope is semantic and hashed. `evaluation-memory` may adapt `workflow-core` into that projection, while `evaluation-mcp` depends only on `evaluation-core` plus MCP/serialization and receives reader/store/trusted-context ports by injection.
