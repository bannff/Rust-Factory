...
- [x] 1. Create `evaluation-core` with versioned definitions, criteria, evidence refs, verdicts, immutable result records, digests, and errors.
...
- [x] Keep `evaluation-core` independent of `workflow-memory`, MCP, and concrete `workflow_core::Run`; `evaluation-memory -> evaluation-core + workflow-core`; `evaluation-mcp -> evaluation-core + MCP` with injected ports.
