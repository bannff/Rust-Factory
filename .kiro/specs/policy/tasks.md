...
- [ ] 7. Specify and migrate Agent MCP to the same trusted-context/Policy boundary; Agent definition sharing remains a separate migration.
...
`policy-mcp`, durable policy persistence/audit projections, Agent MCP Policy compatibility, and tenant-private Agent definitions require their own approved specifications. The process-local Policy memory adapter does not claim durable authorization evidence.

## Evidence

`policy-core` and `policy-memory` have deterministic canonical decision/grant vectors and static-grant validation tests. Workflow and Evaluation Policy compatibility have independent accepted specs, including verified request-bound decisions, deny-before-domain regressions, bounded stdio ingress, required specialist approvals, and `make check` evidence.
