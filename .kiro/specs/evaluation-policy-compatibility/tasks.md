...
- [x] Add `policy-core` only to `evaluation-mcp` and replace the legacy trusted-context constructor seam with the verified Policy compatibility resolver.
...
- [x] Run `cargo test -p evaluation-core -p evaluation-memory -p evaluation-mcp`.
