...
**Span export is explicitly out of scope for this section** (and for issue #57). `opentelemetry::trace::Tracer::build_with_context` has no path to make a built span carry a caller-chosen `span_id`: only the concrete `Tracer`/`IdGenerator` implementation mints span identity, and `SpanContext::new` can only construct a *parent* context handed to `build_with_context`, not force the identity of the span being built. An `OpenTelemetrySpanSink<T: Tracer>` built against this API would either silently mint a different `span_id` than the one `SpanEventV1.context` already carries (a false correlation-fidelity claim, the same class of claim the "no false durability claims" rule already forbids for persistence) or be untestable against that failure mode using `NoopTracer` (which happens to copy the parent context's identity and would mask the defect). Span export requires a lower-level, construct-the-record-don't-mint-it seam (e.g. `opentelemetry_sdk::trace::SpanExporter`/`SpanData`), which is a distinct brick-boundary decision — does it belong in this brick's adapter boundary at all, or one layer out in composition-owned exporter wiring — tracked and re-gated separately as [Issue #62](https://github.com/bannff/Rust-Factory/issues/62). `SpanSink` (§7a.5) remains a real, usable port with no adapter yet; `SpanEventV1`/`TraceContextV1` are unaffected and still fully specified by §7a.

1. `OpenTelemetryMetricSink` SHALL receive an injected `opentelemetry::metrics::Meter` (a concrete type in `opentelemetry` 0.32.0, not a trait — the sink is not generic over it, unlike `OpenTelemetryLogsSink<L: Logger>`). It SHALL NOT construct an SDK, exporter, runtime, batch processor, network client, or shutdown hook, matching §7.1's existing rule. `guarantees()` SHALL report all four `TelemetryGuarantees` fields `false`.
...
- `tests/opentelemetry_contract.rs` — all severity mappings, metadata-only egress for logs and metrics alike, reserved-attribute and baggage exclusion, disabled events, instrument-kind-mismatch rejection, and submit-only guarantees. (Span export is out of scope per §7b; no span-adapter test exists here.)
...
3. **No audit trail.** Logs, spans, and metrics are operational telemetry and cannot replace Workflow/Evaluation evidence. **No baggage export.** No adapter in this brick forwards `TraceContextV1.baggage`; see §7b.4. **No span export adapter.** `SpanSink` (§7a.5) and `SpanEventV1`/`TraceContextV1` (§7a) are real and fully specified, but no `OpenTelemetrySpanSink` adapter exists yet; see §7b's introduction and [Issue #62](https://github.com/bannff/Rust-Factory/issues/62).
