...
* `StaticReferenceCatalog`, `llm_gateway::r#static::StaticProvider`, and `StaticKnowledgeStore` project config data deterministically. The server depends directly on `llm-gateway` with `static`; this supersedes the pre-issue-#34 Agent-owned `StaticModelProvider` composition.
...
```sh
cargo test -p agent-server -p agent -p policy -p llm-gateway -p mcp-transport \
  --features agent/mcp,policy/memory,llm-gateway/static
```
