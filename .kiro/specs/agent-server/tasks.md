...
- [ ] Add the `agent-server` binary package as a root workspace member with only the specified exact direct dependencies; prove no `agent-core` or MCP-library dependency direction changes.
...
- [ ] Run `cargo test -p agent-server -p agent-core -p agent-mcp -p policy-memory -p mcp-transport`, then QA, security, final architecture, final Rust SME, and `make check`.
...
- Architecture and Rust SME approve that the final server is a thin local composition root and leaves `agent-core` unchanged.
