...
For invocation, the adapter converts the verified canonical grant to `EffectiveCapabilityCeilingV1` and calls `LocalAgentRuntime::invoke_with_ceiling` with an explicit Agent-owned `InvocationContextV1` and composition-supplied `llm_gateway::InvocationControl`. Agent core intersects that ceiling with definition policy before resolving tools or building the bounded `llm_gateway::GenerateRequest`; model-requested denied capabilities fail before their ports. The adapter never passes trusted context or Policy decision data into LLM Gateway and does not persist it.

> **Historical supersession:** the original delivered compatibility design referred to Agent-owned `ModelRequest.capability_scope`. Issue #34 removed that synchronous provider API. Current and future composition uses `llm_gateway::LlmProvider`; capability scope and trusted invocation context remain Agent-owned and do not enter gateway DTOs.
