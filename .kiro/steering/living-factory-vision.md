...
| Evaluation | Capability | `evaluation` | `evaluation` with `memory`, evaluator adapter, and `mcp` modules | Implemented (#27): framework-neutral core with opt-in `local`, `memory`, `settings`, `mcp`, and framework-backed `serdes-ai-evals` adapters/features; bounded process-local no-eviction storage; V1 hashes preserved. No runnable Workflow evidence bridge or composition root until #16; not project-ready or runnable. |
