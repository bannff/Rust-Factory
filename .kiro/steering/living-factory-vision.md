...
2. **Execution plane — native Rust.** A deployed agent uses typed ports such as `ToolRegistry`, `MemoryStore`, `KnowledgeStore`, `Sandbox`, and `ModelProvider`, all currently owned by `agent`. Concrete implementations remain replaceable adapters. A `MessageBus` port is intended but does not exist; see the capability taxonomy tracked in GitHub (issues #9 and #11).
